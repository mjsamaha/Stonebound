...
public class World {
	...
	/**
	 * Returns the chunk at the given chunk coordinates, loading/generating if
	 * needed.
	 */
	...
	public int getTileId(int tx, int ty) {
		return getChunk(Math.floorDiv(tx, Chunk.SIZE), Math.floorDiv(ty, Chunk.SIZE))
				.getTileId(Math.floorMod(tx, Chunk.SIZE), Math.floorMod(ty, Chunk.SIZE));
