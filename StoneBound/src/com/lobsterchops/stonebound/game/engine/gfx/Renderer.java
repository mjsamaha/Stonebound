...
public class Renderer {
	 ...
	 * @param bufferWidth  internal resolution width (e.g.
	 *                     {@code ScreenConfig.WIDTH})
	 * @param bufferHeight internal resolution height (e.g.
	 *                     {@code ScreenConfig.HEIGHT})
	 */
	public Renderer(Camera camera, int bufferWidth, int bufferHeight) {
		this.camera = camera;
		this.bufferWidth = bufferWidth;
		this.bufferHeight = bufferHeight;
		this.backBuffer = new BufferedImage(bufferWidth, bufferHeight, BufferedImage.TYPE_INT_ARGB);
	}

	/**
	 * Opens a new frame. Creates a fresh {@code Graphics2D} from the back buffer
	 * and clears it to black. Must be called before any draw method.
	 */
	public void begin() {
	 ...
	 *
	 * <p>
	 * <b>Do NOT dispose {@code swingGraphics}</b> — GamePanel owns it.
	 *
	...
	public void drawSprite(Sprite sprite, float worldX, float worldY) {
		assertFrameOpen();
		sprite.draw(g2, camera.toScreenX(worldX), camera.toScreenY(worldY));
	}
	...
	/**
	 * Draws a tile sprite at tile grid coordinates. Converts tile coords to world
	 * pixels (tile * tileSize) then applies the camera offset.
	 *
	...
	/**
	 * Draws a sprite at a fixed screen position, ignoring the camera. Use this for
	 * HUD elements, overlays, and UI.
	 */
	 ...
	 *
	 * <p>
	 * Intended only for subsystems (e.g. {@code ScreenManager}) that need features
	 * not covered by the named draw methods (AlphaComposite, etc.). Game-logic code
	 * should never call this.
	 *
	 * @return the current frame's {@code Graphics2D}; null if called outside a
	 *         frame
	 */
	...
	private void assertFrameOpen() {
		if (g2 == null) {
			throw new IllegalStateException("Renderer.begin() must be called before any draw method.");
		}
	}
}